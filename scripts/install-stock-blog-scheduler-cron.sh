...
CRON_COMMAND="cd $ROOT_DIR && set -a && . ./.env && set +a && curl -fsS -X POST $BASE_URL/api/stock-blog/scheduler -H \"x-bg-agent-key: \$AGENT_API_KEY\" >> logs/stock-blog-scheduler.log 2>&1 # $CRON_MARKER"
CRON_LINE="$CRON_SCHEDULE $CRON_COMMAND"

echo "Installing BG Company stock blog scheduler cron..."
echo "Schedule: $CRON_SCHEDULE"
echo "Endpoint: $BASE_URL/api/stock-blog/scheduler"
