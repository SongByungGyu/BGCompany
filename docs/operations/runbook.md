...
QA와 결정론적 품질 게이트를 모두 통과한 경우에만 스케줄러가 정책 자동 승인과 Naver Draft Job 생성을 진행합니다. 자동 게시 플래그가 꺼진 작업은 임시저장·사용자 확인으로 끝납니다. 자동 게시 작업도 서버의 `allowPublish`, 로컬 `NAVER_ALLOW_PUBLISH`, 이미지·가독성 검사, 중복·카나리 최종 게이트를 모두 통과해야 하며 로그인·CAPTCHA·보안 확인 또는 게시 오류가 나면 자동 우회나 게시 클릭 재시도 없이 중단합니다.
