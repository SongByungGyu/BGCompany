...
- 제목은 날짜로 시작하지 않는다. 핵심 검색어와 구체 이슈를 먼저 쓰고 확인 기준 날짜는 제목 끝에 "7월 29일"처럼 둔다.
- 첫 3문장 안에서 시장 움직임, 확인된 원인, 투자자가 볼 변수를 답하도록 기획한다.
...
- seoKeywords는 뜻이 겹치지 않는 구체 검색어 5~8개로 작성한다.
...
- "브리핑", "정리", "체크포인트"만 바꾼 유사 제목을 피하고, 최근 발행 제목과 다른 검색 질문을 선택한다.
- 날짜는 "26/07/29"처럼 제목 앞에 쓰지 않고 필요할 때 "7월 29일" 형식으로 제목 끝에 둔다.
- seoKeywords는 검색 의도가 겹치지 않는 5~8개로 제한하고, thumbnailCopy는 핵심 숫자 또는 질문이 보이는 짧은 문구로 쓴다.
- 실제 게시, 외부 발송, 결제, 승인 처리는 하지 않는다.
...
- introduction 첫 3문장 안에서 "무슨 일이 있었는지 → 확인된 원인 → 투자자가 볼 변수"를 먼저 답한 뒤 본문에서 근거를 설명한다.
