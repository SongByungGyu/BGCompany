...
const RESULT_EVENT_STUDIES = [
  {
    ...
    topic: "엔비디아 공식 실적 자료와 SEC 제출에서 매출·데이터센터 매출·EPS·다음 분기 가이던스를 확인합니다. 데이터센터 수요 → AI 가속기 출하 기대 → HBM 수요 기대 → 삼성전자·SK하이닉스 투자심리와 국내 반도체 수급 순서로 전달 경로를 설명하되, 각 단계는 해당 시점의 검증 기사와 실제 시장 자료가 있을 때만 연결합니다. 시간외 주가와 시장 예상치는 출처와 비교 시각이 확인된 수치만 사용합니다.",
