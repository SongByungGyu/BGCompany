...
function PaperTradingPanel({
  ...
  return <div className="paper-trading-dashboard">
    ...
    {automation ? <section className={`paper-automation ${automation.status}`}>
      <div><span>AUTOMATIC PAPER CYCLE</span><h3>{automation.enabled ? "완전 자동 모의투자 실행 중" : "자동 실행 꺼짐"}</h3><p>KIS 읽기 전용 일봉 → 분기 모멘텀 계획 고정 → 다음 거래일 시가 가상 체결 → 일별 손익 저장{automation.trialStartMarketDate && automation.trialEndMarketDate ? ` · 실험 ${automation.trialStartMarketDate} ~ ${automation.trialEndMarketDate}` : ""}</p></div>
      <dl>
    ...
    {response.strategy ? <section className="paper-rotation-plan">
      ...
      <footer><span>종목군 {response.strategy.universeSize}개 · 현금 목표 {response.strategy.cashTargetPercent}% · 시작 {response.strategy.startMode === "MID_QUARTER_BASELINE" ? "분기 중간 기준선" : "분기 시작"}</span><strong>SPY 대비 {response.strategy.excessReturnPercent == null ? "집계 대기" : formatPercent(response.strategy.excessReturnPercent)}</strong></footer>
    </section> : null}
    ...
    <div className="paper-trading-columns">
      <section className="portfolio-section compact"><header><div><span>VIRTUAL POSITIONS</span><h2>모의 보유종목</h2><p>실제 보유종목과 분리된 가상 체결 결과입니다.</p></div><b>{response.positions.length}종목</b></header>
        <div className="paper-position-list">{response.positions.length ? response.positions.map((position) => <article key={position.id}><div><b>{position.symbol}</b><strong>{position.name}</strong><small>{position.targetRank ? `분기 순위 ${position.targetRank}위 · 목표 ${position.targetWeightPercent}%` : position.strategy}</small></div><dl><div><dt>수량</dt><dd>{position.quantity}주</dd></div><div><dt>진입/현재</dt><dd>${Number(position.entryPriceUsd).toFixed(2)} / ${Number(position.lastPriceUsd).toFixed(2)}</dd></div><div><dt>종료 규칙</dt><dd>{position.targetRank ? "다음 분기 교체" : `$${Number(position.stopPriceUsd).toFixed(2)}`}</dd></div><div><dt>평가손익</dt><dd className={(number(position.unrealizedPnlKrw) ?? 0) >= 0 ? "gain" : "loss"}>{formatAmount(position.unrealizedPnlKrw, "KRW")} · {formatPercent(position.returnPercent)}</dd></div></dl></article>) : <p className="portfolio-empty-copy">아직 체결된 모의 포지션이 없습니다. 선정된 순서대로 하루 최대 3종목을 다음 거래일 시가 기준으로 가상 체결합니다.</p>}</div>
      </section>
